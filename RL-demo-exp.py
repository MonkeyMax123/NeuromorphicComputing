...
class Cell(GridCell):
    def color(self):
        return "pink" if self.wall else None
...
with model:
    ...
    def move(t, x, my_world=world_cfg):
        ...
        if not success:
            my_world.agent.turn_right()
            my_world.agent.turn_left()
            #my_world.agent.turn
            my_world.agent.color = "red"
            return 0
        else:
            my_world.agent.color = "blue"
            return turn_bias + abs(speed)
    ...
    print("Thalamus output:", thal.output[0])
    nengo.Connection(thal.output[1], movement, transform=[[0], [1]])
    print("Thalamus output:", thal.output[1])
    nengo.Connection(thal.output[2], movement, transform=[[0], [-1]])
    print("Thalamus output:", thal.output[2])
    #nengo.Connection(thal.output[3], movement, transform=[[-1], [0]])
